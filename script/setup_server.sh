...
USER=`deploy`
APP_ROOT=/srv/www/hepte
DB_NAME=hepte_production
...
# Nginx config
sudo cp config/nginx.example.conf /etc/nginx/sites-available/hepte
sudo sed -i "s|root .\+;|root $APP_ROOT/current/public;|" /etc/nginx/sites-available/hepte
sudo ln -s /etc/nginx/sites-available/hepte /etc/nginx/sites-enabled
